...
def _wordpress_verify_tls() -> bool:
    # The current voyayaha.com hosting certificate can fail validation from
    # non-browser clients. Keep this configurable and default to False for
    # the Render server-to-server proxy.
    return os.getenv("VOYAYAHA_WORDPRESS_VERIFY_TLS", "false").strip().lower() in {
        "1", "true", "yes", "on"
    }
...
async def travel_memories_proxy(per_page: int = Query(100, ge=1, le=100)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="WordPress Travel Memories API could not be reached from Render.",
        )
...
async def travel_memory_proxy(
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="WordPress Travel Memory API could not be reached from Render.",
        )
...
async def api_travel_memory_proxy(
    ...
    return await travel_memory_proxy(
        title, location, latitude, longitude, date, description, photo
    )
